...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`partner_tiers\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`label\` text NOT NULL,
  	\`order\` numeric DEFAULT 0 NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`settings\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`site_name\` text DEFAULT 'Argo' NOT NULL,
  	\`alternate_name\` text,
  	\`meta_title\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`settings_contact_form_idx\` ON \`settings\` (\`contact_form_id\`);`)
  await db.run(sql`CREATE INDEX \`settings_organization_logo_idx\` ON \`settings\` (\`organization_logo_id\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`partners_id\` integer REFERENCES partners(id);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`partner_tiers_id\` integer REFERENCES partner_tiers(id);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`partner_logos_id\` integer REFERENCES partner_logos(id);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "projects_id", "posts_id", "media_id", "users_id") SELECT "id", "order", "parent_id", "path", "projects_id", "posts_id", "media_id", "users_id" FROM \`payload_locked_documents_rels\`;`)
